  ...
  describe('updateVersion', () => {
    describe('never initialized', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          byUrl: {
            'https://unpkg.com/moonwalkerswap-default-token-lists@1.0.6/src/tokens/avalanche.json': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
            },
            'https://unpkg.com/moonwalkerswap-default-token-lists@latest': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
      ...
      it('clears the current lists', () => {
        expect(
          store.getState().byUrl['https://unpkg.com/moonwalkerswap-default-token-lists@1.0.6/src/tokens/avalanche.json']
        ).toBeUndefined()
        expect(store.getState().byUrl['https://unpkg.com/moonwalkerswap-default-token-lists@latest']).toBeUndefined()
      })
        ...
        Object.keys(s.byUrl).forEach(url => {
          if (url === DEFAULT_TOKEN_LIST_URL) {
            expect(s.byUrl[url]).toEqual({
              error: null,
              current: MOONWALKERSWAP_DEFAULT_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
            })
          } else {
    ...
    describe('initialized with a different set of lists', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          ...
          selectedListUrl: undefined,
          lastInitializedDefaultListOfLists: ['https://unpkg.com/moonwalkerswap-default-token-lists@latest']
        })
        store.dispatch(updateVersion())
      })
      ...
      it('each of those initialized lists is empty', () => {
        ...
        Object.keys(byUrl).forEach(url => {
          if (url !== 'https://unpkg.com/moonwalkerswap-default-token-lists@1.0.6/src/tokens/avalanche.json') {
